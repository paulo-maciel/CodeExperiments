...
void Buffer::create(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer, VkDeviceMemory &bufferMemory)
{
  ...
  allocInfo.memoryTypeIndex = findMemoryType(device_->getPhysicalDevice(), memRequirements.memoryTypeBits, properties);
...
uint32_t Buffer::findMemoryType(VkPhysicalDevice device, uint32_t typeFilter, VkMemoryPropertyFlags properties) {
